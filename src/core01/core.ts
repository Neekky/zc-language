...
let tokenArray: Token[] = [
  ...
  { kind: TokenKind.Identifier, text: "println" },
  { kind: TokenKind.Seperator, text: "(" },
  { kind: TokenKind.StringLiteral, text: "is foo!" },
  { kind: TokenKind.Seperator, text: ")" },
  { kind: TokenKind.Seperator, text: ";" },

  { kind: TokenKind.Identifier, text: "println" },
  { kind: TokenKind.Seperator, text: "(" },
  { kind: TokenKind.StringLiteral, text: "多级函数没有运行!" },
  { kind: TokenKind.Seperator, text: ")" },
  { kind: TokenKind.Seperator, text: ";" },
...
class Parser {
  ...
  parseFunctionBody(): FunctionBody | null | void {
    ...
    if (t.text == "{") {
      ...
      console.error("Expecting '{' in FunctionBody, while we got a " + t.text);
      return;
    }
...
class RefResolver extends AstVisitor {
  ...
  private findFunctionDecl(prog: Prog, name: string): FunctionDecl | null {
    for (let x of prog?.stmts) {
      ...
      if (typeof functionDecl.body === "object" && functionDecl.body.stmts.length > 1) {
        console.log("需要消解代码", functionDecl)
        functionDecl.body.stmts.forEach(ele => {
          if (!ele.deal) {
            ele.deal = true;
            this.findFunctionDecl(prog, ele.name);
          }
        })
      }
